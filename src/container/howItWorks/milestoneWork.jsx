...
class MilestoneWork extends Component {
  constructor(props) {
    super(props);
    this.state = {};
  }
        ...
        <section className="about_sec">
          <div className="container">
            <h3 className="title_h3 text-center">
              {languageType.MILESTONE_TEXT}, {languageType.HOW_IT_WORKS}
            </h3>
            <p className="text">
              마일스톤 형식의 프로젝트는 프리랜서와 전체 프로젝트 금액과 시간을
              산정한 후, 그에 따라서 대금을 지급하는 형식의 프로젝트입니다. 예를
              들어 전체 프로젝트가 US$10,000.00이라고 했을때 이를 주당 또는
              합의된 기간에 따라 대금을 작은 금액으로 분할하여 프리랜서가 제공한 업무를
              사전 확인하고 대금을 지급함으로써 프로젝트를 안전하게 진행시킬수
              있습니다.
            </p>
            <p className="text">
              프로젝트 관리툴을 활용하여 프로젝트를 에피소드와 스토리로 좀 더
              세분하여 예산을 각 프로세스마다 산정하고 타스크 및 참여 인원을 관리하며 전체
              프로젝트의 프로세스를 관리하실수 있습니다. 전체 기획이 완성되어
              디테일한 부분까지 요구사항이 확정되었다면 대형 프로젝트에
              적용하기에 매우 좋은 프로젝트 형태입니다.
            </p>
            <p className="text"></p>
...
function mapStateToProps(state) {
  return { languageType: state.languageReducer.languageType };
}
function mapDispatchProps(dispatch) {
  return {};
}

export default connect(mapStateToProps, mapDispatchProps)(MilestoneWork);
